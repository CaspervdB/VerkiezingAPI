...
import { Election, ElectionDbModel, mapToDb, mapToElection } from "../models/election";
...
export const dbUpdateElection = async (election: Election): Promise<Election> => {
    ...
    console.log(1);
    await collection.updateMany({ _id: new ObjectId(election.id) }, { $set: { ...mapToDb(election)! }});
    console.log(2);
